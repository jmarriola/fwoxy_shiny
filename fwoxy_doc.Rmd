...
$$
$GPP$ is calculated by multiplying $I$, the photosynthetic active radiation (W/m<sup>2</sup>), by the light efficiency $\alpha$ ((mmol/m<sup>3</sup>) / (W/m<sup>2</sup>)). The gas exchange term, $GASEX$, is calculated by first dividing the gas transfer velocity, $K_{O_2}$ (m/s), by the height of the water column, $H$ (m), and then multiplying by the oxygen saturation deficit, $D$ (mmol/m<sup>3</sup>). $K_{O_2}$ is estimated using the parameterization of Wanninkhof (2014) based on wind speed (m/s), water temperature ($^{\circ}$ C) and salinity (ppt). $D$ is calculated using salinity and water temperature based on Millero and Poisson (1981) for seawater density (kg/m<sup>3</sup>) and Garcia and Gordon (1992) for oxygen saturation (mmol/m<sup>3</sup>).
<br>
<br>
<br>
...
<br>
<br>
The top parameter is the initial oxygen concentration (mmol/m<sup>3</sup>), whereas the remaining parameters are constants. They are light efficiency ($\alpha$, (mmol/m<sup>3</sup>) / (W/m<sup>2</sup>)), ecosystem respiration ($ER$, mmol/m<sup>3</sup>/day), depth of the water column ($H$, meters), salinity (ppt), water temperature ($^{\circ}$ C), and wind speed (m/s). $I$ (Eq. 2) is based on a sin function that reaches a maximum of 400 W/m<sup>2</sup> at noon every day and is zero between the hours of 6:00 pm to 6:00 am. The length of the model run is 6 days at 15 minute intervals. 
<br>
<br>

#### Outputs
On the right hand side are the output plots for $fwoxy$. The top panel shows the oxygen concentration (mmol/m<sup>3</sup>, dark blue) within the water column every 15 minutes over the 6 day model run. The bottom panel shows the $TROC$ (light blue), $GPP$ (yellow), $GASEX$ (red), and $ER$ (purple) fluxes at rates of mmol/m<sup>3</sup>/day for every 15 minutes over the 6 day model run. All outputs automatically refresh whenever a slider is changed. 
<br>
<br>
